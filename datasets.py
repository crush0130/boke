...
class CathCanonicalAnglesDataset(Dataset):
    ...
    def __init__(
        ...
        dssp_cache_file: Optional[str] = "/media/dell/新加卷/zym/foldingdiff/foldingdiff/cached_dssp_labels.pkl",
    ) -> None:
        super().__init__()
        ...
        self.dssp_labels: Optional[Dict[str, torch.Tensor]] = None
        if dssp_cache_file and os.path.exists(dssp_cache_file):
            logging.info(f"Loading precomputed DSSP labels from {dssp_cache_file}")
            with open(dssp_cache_file, "rb") as f:
                self.dssp_labels = pickle.load(f)
        else:
            logging.warning("No precomputed DSSP label file found. DSSP labels will be missing.")
    ...
    def _get_dssp_labels(self, fname: str, length: int) -> List[int]:
        if self.dssp_labels is not None:
            key = os.path.basename(fname)
            if key in self.dssp_labels:
                labels = self.dssp_labels[key].tolist()
                return labels[:length] if len(labels) > length else labels + [3] * (length - len(labels))
        return [3] * length
    def __getitem__(
        ...
        fname = self.structures[index]["fname"]
        # FIX: coords is flattened [L*3, 3], reshape to [L, 3, 3]
        if coords.ndim == 2 and coords.shape[1] == 3:
            if coords.shape[0] % 3 != 0:
                raise ValueError(
                    f"coords length {coords.shape[0]} not divisible by 3"
                )
            coords = coords.reshape(-1, 3, 3)
        if angles.shape[0] != coords.shape[0]:
            print("=== DATASET DEBUG ===")
        ...
        # 只用 CA 原子
        ca = coords[:, 1, :]  # [L, 3]
        # 构造 residue pairwise 相对向量
        rel_vec = ca[:, None, :] - ca[None, :, :]  # [L, L, 3]
        # 距离
        dist = torch.norm(rel_vec, dim=-1)  # [L, L]
        # 可选：距离裁剪 / 归一化（强烈建议）
        dist = torch.clamp(dist, max=20.0) / 20.0

        single_res_rel = dist  # [L, L]
        assert single_res_rel.shape == (L, L)

        angles = torch.from_numpy(angles).float()

        # DSSPzhuru
        dssp_feat = self._get_dssp_labels(fname, length=l)
        dssp_feat = torch.tensor(dssp_feat, dtype=torch.long)
        if len(dssp_feat) < self.pad:
            padded = torch.full((self.pad,), fill_value=3, dtype=torch.long)
            padded[:len(dssp_feat)] = dssp_feat
            dssp_feat = padded
        elif len(dssp_feat) > self.pad:
            dssp_feat = dssp_feat[:self.pad]
        retval = {
            "angles": angles,
            "coords": coords,
            "attn_mask": attn_mask,
            "single_res_rel": single_res_rel,
...
class CathCanonicalAnglesOnlyDataset(CathCanonicalAnglesDataset):
    ...
    feature_names = {"angles": ["phi", "psi", "omega", "tau", "CA_C_1N", "C_1N_1CA"]}
