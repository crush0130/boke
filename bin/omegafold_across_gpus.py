...
def run_omegafold(input_fasta: str, outdir: str, gpu: int, weights: str = ""):
    ...
    OMEGAFOLD_BIN = "/home/dell/anaconda3/envs/foldingdiff/bin/omegafold"
    assert os.path.exists(OMEGAFOLD_BIN), f"OmegaFold not found: {OMEGAFOLD_BIN}"

    logging.info(
        f"Running omegafold on {input_fasta} > {outdir} | gpu={gpu}"
    )
    ...
    if weights:
        ...
        cmd += ["--weights_file", weights]

    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = str(gpu)

    bname = os.path.splitext(os.path.basename(input_fasta))[0]
    log_file = os.path.join(outdir, f"omegafold_{bname}_gpu_{gpu}.stdout")

    with open(log_file, "wb") as sink:
        subprocess.run(cmd, stdout=sink, stderr=subprocess.STDOUT, env=env, check=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default=os.path.join("/media/dell/新加卷/zym/foldingdiff/out_dir/proteinmpnn_residues"),
    ...
    parser.add_argument(
        ...
        "--outdir",
        type=str,
        default=os.path.abspath(os.path.join("/media/dell/新加卷/zym/foldingdiff/out_dir", "omegafold_predictions")),
