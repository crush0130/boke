...
def build_parser():
    ...
    parser.add_argument(
        ...
        "--predicted",
        type=str,
        default=os.path.join('/media/dell/新加卷/zym/foldingdiff/out_dir', "sampled_pdb"),
    ...
    parser.add_argument(
        ...
        "--folded",
        type=str,
        default=os.path.join('/media/dell/新加卷/zym/foldingdiff/out_dir', "omegafold_predictions"),
...
def main():
    ...
    # 新增：生成带序列信息且只保留 scTM >= 0.5 的 JSON
    xin_sctm_scores_mapping = {}

    for orig_name, scTM in sctm_scores_mapping.items():
        if scTM >= 0.5:  # 只保留 scTM >= 0.5 的条目
            # 获取最佳匹配的折叠 PDB 文件名
            best_ref = sctm_scores_reference.get(orig_name)
            if best_ref is None:
                logging.warning(f"No reference file for {orig_name}, skipping")
                continue
            # 提取序列标签（例如 "1_residues_0"）
            base = os.path.basename(best_ref)
            base_no_ext = os.path.splitext(base)[0]
            # 假设命名规则为 "{orig_name}_{seq_tag}.pdb"
            if base_no_ext.startswith(orig_name + "_"):
                seq_tag = base_no_ext[len(orig_name) + 1:]  # 去掉 "orig_name_"
            else:
                # 如果命名不符合预期，直接使用整个 basename 作为标签
                seq_tag = base_no_ext
            new_key = f"{orig_name}_{seq_tag}"
            xin_sctm_scores_mapping[new_key] = scTM

    # 写出新的 JSON
    with open(args.outprefix + "_xin.json", "w") as sink:
        json.dump(xin_sctm_scores_mapping, sink, indent=4)

    logging.info(
        f"Written {len(xin_sctm_scores_mapping)} entries with scTM >= 0.5 to {args.outprefix}_xin.json"
    )
